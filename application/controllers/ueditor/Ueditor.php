...
class ueditor extends MY_Controller 
{
    ...
    public function init()
	{
		...
		$CONFIG = json_decode(preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents($config['base_url']."/lib/ueditor/config.json")), true);
		...
		$wurl=config_item('base_url');
		$CONFIG['imageUrlPrefix']=$wurl;
		$CONFIG['scrawlUrlPrefix']=$wurl;
		$CONFIG['snapscreenUrlPrefix']=$wurl;
		$CONFIG['catcherUrlPrefix']=$wurl;
		$CONFIG['videoUrlPrefix']=$wurl;
		$CONFIG['fileUrlPrefix']=$wurl;
		$CONFIG['imageManagerUrlPrefix']=$wurl;
	    $CONFIG['fileManagerUrlPrefix']=$wurl;
		...
		switch ($action) {
			...
			case 'uploadimage':
			{
				...
				//error_log($result,3,$_SERVER['DOCUMENT_ROOT']."/log.txt");
				break;
			}	
