...
class Batches extends MY_Controller 
{
	...
	public function batches_list()
	{
		...
		{
			//非模糊字段 搜索
			$search_key      = array('id','status','tax_status','freight_status');
			//模糊字段 搜索
	...
	public function batches_edit()
	{
		...
		if ( !empty( $_POST['id'] ) ) 
		{
			...
			else
			{
				...
				//判断批次中的订单数
				$order_info = $this->Base_Batches_model->get_batches_info('order_num,status',array('id'=>$_POST['id']));
				$order_num  = $order_info['order_num'];
				$status     = $order_info['status'];

				if($status==3)
				{
					if($this->input->post('post_order_num',true)!=$order_num){
						$msg = array(
