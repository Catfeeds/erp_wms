...
CREATE TABLE IF NOT EXISTS `dferp_fedex_pakge` (
  ...
  `fedex_send_con` text COMMENT '推送信息',
  PRIMARY KEY (`id`),
  KEY `fedex_index` (`fedex_index`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='包裹信息' AUTO_INCREMENT=1 ;


CREATE TABLE IF NOT EXISTS `dferp_fedex_user` (
  `id` int(8) NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `type`  tinyint(1) NOT NULL   DEFAULT '1'  COMMENT '1=国外 2=国内',
  ...
  `Address_City` varchar(50) DEFAULT NULL COMMENT '地址',
  ...
  PRIMARY KEY (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='收发人信息' AUTO_INCREMENT=1 ;
